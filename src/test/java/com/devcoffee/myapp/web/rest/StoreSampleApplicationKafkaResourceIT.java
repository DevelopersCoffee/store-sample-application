...
import com.devcoffee.myapp.config.KafkaProperties;
...
class StoreSampleApplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        StoreSampleApplicationKafkaResource kafkaResource = new StoreSampleApplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/store-sample-application-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/store-sample-application-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
